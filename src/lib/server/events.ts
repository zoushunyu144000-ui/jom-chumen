...
async function ensureSeeded() {
  ...
  for (const event of EVENT_SEED) {
    const exists = await sql<{ id: string }>`
      select id from events where id = ${event.id} limit 1
    `;
  ...
  for (const club of CLUB_SEED) {
    const exists = await sql<{ id: string }>`
      select id from clubs where id = ${club.id} limit 1
    `;
    if (!exists[0]) {
      await sql`
        insert into clubs (id, user_id, name, bio, city, cover_url)
        values (${club.id}, ${"seed"}, ${club.name}, ${club.bio}, ${club.city}, ${club.coverUrl})
      `;
    }
    for (const slug of club.eventSlugs) {
      await sql`
        update events
        set club_id = ${club.id}, host_name = ${club.name}
        where slug = ${slug} and (club_id is null or club_id = ${club.id})
      `;
    }
  }
  await sql`
...
export const listEvents = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const rows = await sql.query<EventRow>(
      `${eventSelect} order by e.starts_at asc`,
    );
    ...
    return rows
      ...
      .filter((event) => {
        ...
        const hay = `${event.title} ${event.subtitle} ${event.venue} ${event.hostName} ${event.clubName ?? ""}`.toLowerCase();
...
export const getEventBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const sql = await getSql();
    const rows = await sql.query<EventRow>(
      `${eventSelect} where e.slug = $1 limit 1`,
      [data.slug],
    );
...
export const createRegistration = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const counted = await sql<{ n: number }>`
      select count(*)::int as n from registrations where apply_no like ${`${prefix}%`}
    `;
    ...
    return {
      ...
      full: event.remaining < data.seats,
      event,
    };
  });
  ...
  .handler(async ({ data }) => {
    ...
    const rows = code
      ? await sql<RegRow>`
          ...
          where code = ${code} or apply_no = ${code}
    ...
    for (const row of rows) {
      const eventRows = await sql.query<EventRow>(
...
export const cancelApplication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await sql`
      update registrations set payment_status = 'cancelled' where id = ${row.id}
    `;
  });
