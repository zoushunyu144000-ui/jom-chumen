  ...
  return (
    <main className="pb-28">
      <div className="relative">
        <EventGallery images={images} alt={event.title} />
        <Link
          to="/"
          className="absolute left-3 top-3 z-10 flex size-11 items-center justify-center rounded-full bg-paper/90 text-ink shadow-card"
          aria-label="返回"
        >
      ...
      <section className="px-4 pt-4">
        ...
        <ul className="mt-4 space-y-3 rounded-xl bg-surface p-4 text-sm shadow-card">
          ...
          <li className="flex gap-3">
            <Ticket className="mt-0.5 size-4 shrink-0 text-muted" />
          ...
          <li className="flex gap-3">
            ...
            <span>
              ...
              {event.remaining > 0 ? ` · 还剩 ${event.remaining}` : " · 名额已满"}
            </span>
          </li>
        </ul>

        {hasMap ? (
          <EventMap lat={event.lat as number} lng={event.lng as number} label={event.venue} className="mt-4" />
        ) : null}
        ...
        {event.clubId && event.clubName ? (
          <Link to="/clubs/$id" params={{ id: event.clubId }} className="mt-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted">查看俱乐部</p>
            </div>
          </Link>
        ) : null}
        ...
        {intro ? (
          <>
            ...
            <p className="mt-2 whitespace-pre-wrap text-[15px] leading-relaxed text-ink-soft">{intro}</p>
          </>
        ) : null}
