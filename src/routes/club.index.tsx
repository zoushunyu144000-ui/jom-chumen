    ...
    <main className="px-4 pb-8 pt-[max(1rem,env(safe-area-inset-top))]">
      ...
      <Tabs defaultValue="inbox" className="mt-5">
        ...
        <TabsContent value="events">
          {events === null ? <Skeleton className="mt-2 h-24 rounded-xl" /> : events.length === 0 ? (
                  ...
                  <li key={event.id} className="rounded-xl bg-surface p-2 shadow-card">
                    <div className="flex gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted">{formatWhen(event.startsAt, event.currency)} · 已录 {event.booked}/{event.capacity}</p>
                        {pending > 0 ? <p className="mt-1 text-xs font-medium">{pending} 条待确认</p> : null}
                      </div>
                    </div>
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      <Button asChild variant="outline" size="sm"><Link to="/manage/$eventId" params={{ eventId: event.id }}>审核报名</Link></Button>
        ...
        <TabsContent value="clubs">
          {clubs === null ? <Skeleton className="mt-2 h-24 rounded-xl" /> : clubs.length === 0 ? (
            <div className="rounded-xl bg-surface px-4 py-10 text-center shadow-card"><p className="font-medium">还沠有俱乐部</p></div>
          ) : (
            <ul className="space-y-2">
              {clubs.map((club) => (
