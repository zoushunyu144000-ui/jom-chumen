...
export const Route = createFileRoute("/ticket/$code")({
  ...
  notFoundComponent: () => (
    <main className="px-6 py-20 text-center">
      ...
      <Link to="/tickets" className="mt-4 inline-block text-sm text-muted underline">
        打开票夹
      </Link>
    </main>
  ),
...
function TicketPage() {
    ...
    return (
      <main className="px-4 pb-10 pt-[max(1rem,env(safe-area-inset-top))]">
        ...
        <p className="mt-1 text-sm text-muted">
          当前状态：{applyStatusLabel(ticket.paymentStatus)}。提交申请不等于报名成功。
        </p>
        <Button asChild className="mt-5 w-full">
          <Link to="/apply/$code" params={{ code: ticket.code }}>
            查看申请与付款指引
          </Link>
        </Button>
      </main>
  ...
  return (
    <main className="px-4 pb-10">
      ...
      <p className="mt-4 text-center text-xs text-muted">{ticket.phoneMasked}</p>
    </main>
  );
}
